...
const filteredAndSortedCampaigns = computed(() => {
  let filtered = campaigns.value.filter((campaign) => {
    ...
    const niche = campaign.niche?.toLowerCase() || '';
    
    return (
      name.includes(query) || niche.includes(query)
    );
  });
...
<template>
  ...
  <div style="margin-top: 5em" class="container mt-10">
    <h2 style="font-weight: bold;" class="text-center mb-2">Campaigns List</h2>
        ...
        >
          ...
          <td>
            <button
              ...
              @click="sendAdRequest(campaign.id)"
            >
              Send Ad Request
            </button>
          </td>
