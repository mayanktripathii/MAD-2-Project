...
<template>
  <div style="margin-top: 5em;">
    <Logout DashboardTitle="Sponsor dashboard"></Logout>

    <nav class="navbar navbar-expand-lg navbar-light bg-light">
    <!-- Left-Aligned Button -->
    <div class="navbar-left">
      <button class="btn btn-primary" @click="navigateTo('search_influencer')">
        Search Influencers
      </button>
    </div>
    ...
    <div class="container">
      ...
      <h1 style="font-weight: bold">Welcome, {{ userName }}</h1>
    </div>
      ...
      <div v-if="campaignData.length" class="row">
        ...
        <table class="table table-striped table-bordered campaign-list">
          ...
          <tbody>
            <tr v-for="data in campaignData" :key="data.id">
              <td>{{ data.name }}</td>
...
.navbar-left .btn {
  margin-right: 10px; /* Add spacing to the right of the left button */
}

.navbar-right .btn {
  margin-left: 10px; /* Add spacing to the left of the right button */
}
