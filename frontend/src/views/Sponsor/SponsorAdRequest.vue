...
<template>
  ...
  <div class="container mt-3">
    <h2 class="text-center mb-4" style="font-weight: bold;">{{ campaign.name || "Campaign Details" }}</h2>
      ...
      <table class="table table-striped table-bordered">
        ...
        <tbody>
          <tr v-for="request in adRequests" :key="request.id">
            ...
            <td>{{ formatCurrency(request.negotiated_amount) }}</td>
            <td>{{ request.negotiation_status }}</td>

            <td>
              <button
                class="btn btn-primary btn-sm" style="margin-right: 0.5em;"
                @click="editAdRequest(request.id)"
              >
                Edit
              </button>
              <button
