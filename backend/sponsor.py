...
from model import db, User, Sponsor, Campaign, AdRequest, Influencer
...
def login():
    ...
    if user and user.check_password(password):
        sponsor_data = Sponsor.query.filter_by(user_id = user.id).first()
        
        # print(sponsor_data.to_dict())
        if sponsor and not sponsor_data.status=="Approved":
        ...
        db.session.commit()
        return jsonify({"token": token ,  "role": user.role, "user_id": sponsor_data.id}), 200
...
def export_campaigns():
    try:
        ...
        campaigns = Campaign.query.filter_by(sponsor_id=sponsor_data.id).all()
        ...
        def generate():
            ...
            # Write CSV header
            writer.writerow(["Name", "Description", "Budget", "Start Date", "End Date", "Goals", "Visibility"])
            # Write campaign data
...
def adrequest_campaign(campaign_id):
    try:
        # Fetch ad requests, along with the influencer, campaign, and sponsor details.
        adrequests = (
        ...
        for adrequest, influencer, campaign, sponsor in adrequests:
            ...
            # Extract negotiation data directly from AdRequest model
            adrequest_info['negotiation'] = {
                'negotiation_status': adrequest.negotiation_status,
                'negotiated_amount': adrequest.negotiated_payment_amount
            }
