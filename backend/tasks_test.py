...
from model import User, AdRequest, Sponsor, Campaign
...
def test_send_daily_reminders(test_app, test_celery):
    ...
    user = User(
        username='testuser',
        email='21f3002290@ds.study.iitm.ac.in',
        role='influencer',
    ...
    ad_request = AdRequest( campaign_id=1,influencer_id=user.id, status='pending',sponsor_id=1)
...
def test_send_monthly_report(test_app, test_celery):
    ...
    report_path = os.path.join('static', 'reports', f'monthly_report_{sponsor.id}.html')
...
def test_export_campaigns_as_csv(test_app, test_celery):
    ...
    # Call the task
    result = export_campaigns_as_csv.apply(args=[sponsor.id])
    assert result.status == 'SUCCESS'

    # Verify CSV file generated
    csv_file_path = os.path.join('static', 'reports', f'campaigns_{sponsor.id}.csv')
